...
- **changelog** - update `CHANGELOG.md` only for **user-facing** changes (behaviour, CLI, public
  ...
  Files under `ffmpeg_progress/`, `tests/`, or version changes in `pyproject.toml` are **candidates**
...
For example, if a commit contains `ffmpeg_progress/commands/main.py`,
...
For Python files, strip the `ffmpeg_progress/` prefix and replace `/` with `.` (like module imports).

- Python file `ffmpeg_progress/media.py` → `media:`.
- Multiple files under `ffmpeg_progress/commands/` → `commands:`.
- Single command file `ffmpeg_progress/commands/admin.py` → `commands.admin:`.
