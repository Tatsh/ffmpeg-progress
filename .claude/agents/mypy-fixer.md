...
Fixes mypy type errors and eliminates `Any` usage in the ffmpeg-progress project.
...
1. Fix all mypy errors reported by `uv run mypy ffmpeg_progress`
...
Place new TypedDict classes in `ffmpeg_progress/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`ffmpeg_progress/typing.py` when shared.
...
1. Run `uv run mypy ffmpeg_progress` and capture all errors.
...
4. Run `uv run mypy ffmpeg_progress` again to verify fixes.
